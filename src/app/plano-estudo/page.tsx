...
export default function PlanoEstudoPage() {
  ...
  const getStudyMinutesForDate = (date: Date): number => {
    const dateString = date.toLocaleDateString('pt-BR', {timeZone: 'America/Porto_Velho'});
    const dayOfWeek = date.getDay(); // 0=Domingo, 6=Sábado

    if (dateString === '15/10/2025' || dateString === '30/10/2025') return 0;
    
    const travelStart = new Date('2025-10-23T00:00:00-04:00');
    const travelEnd = new Date('2025-10-29T23:59:59-04:00');
    if (date >= travelStart && date <= travelEnd) return 3 * 60;

    const vacationStart = new Date('2025-10-20T00:00:00-04:00');
    const vacationEnd = new Date('2025-11-09T23:59:59-04:00');
    if (date >= vacationStart && date <= vacationEnd) return 9 * 60;
    
    if (dayOfWeek >= 1 && dayOfWeek <= 5) return 2.5 * 60;
    if (dayOfWeek === 6) return 9 * 60;
    if (dayOfWeek === 0) return 7 * 60;
                        ...
                        <div className="pl-4">
                            ...
                            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground mb-3">
                                ...
                                <div className="flex items-center gap-2" title="Status de conclusão do módulo">
                                  {(item.isPartial && item.durationStudied < item.totalModuleDuration) ? (
                                    <>
