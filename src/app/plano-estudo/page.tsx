...
export default function PlanoEstudoPage() {
    // Definindo o estado inicial para a nova primeira aba do array de estudos: Sáb, 04/10
    ...
    const getTabStyle = (day: StudyDay) => {
        // Estilo especial para a aba ativa que é o dia de hoje (04/10)
        ...
        if (day.isSpecial) return "bg-purple-100 border-purple-300 text-purple-700 hover:bg-purple-200"
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            {/* Sidebar fictícia para layout (mantenha a importação) */}
            {/* Se você não tiver o componente real, adapte ou remova a linha abaixo */}
            {/* <Sidebar /> */}
            <main className="flex-1 md:ml-0 p-4 sm:p-8">
                <div className="max-w-6xl mx-auto">
                    <h1 className="text-3xl font-bold text-slate-800 mb-2">
                        Plano de Estudos - Jornada Full-Stack (v14 - Sáb, 04/10)
                    </h1>
                    <p className="text-slate-600 mb-8">
                        Seu cronograma recalculado, com o novo início em <strong>04 de Outubro de 2025</strong> e o progresso detalhado dos cursos.
                    </p>

                    {/* Legenda de cores */}
                    <div className="flex flex-wrap gap-4 text-xs mb-6 text-slate-600">
                        <span className="flex items-center"><span className="w-3 h-3 rounded-full bg-indigo-500 mr-2"></span> Dia Atual</span>
                        ...
                        <span className="flex items-center"><span className="w-3 h-3 rounded-full bg-purple-500 mr-2"></span> Evento Especial</span>
                        <span className="flex items-center"><span className="w-3 h-3 rounded-full bg-pink-500 mr-2"></span> Descanso (Aniversário)</span>
                                ...
                                {activeDay.isIntensive && !activeDay.isVacation && (
                                    <div className="mt-6 bg-orange-50 p-4 rounded-lg border border-orange-200">
                                        <p className="text-orange-800 text-sm font-semibold flex items-center">
                                            ...
                                            Ritmo Intensivo: Dia de estudo estendido. Mantenha a hidratação, faça pausas regulares e aproveite o foco extra!
                                        </p>
                                    </div>
                                )}
                                ...
                                {activeDay.isVacation && activeDay.isIntensive && (
                                    <div className="mt-6 bg-blue-50 p-4 rounded-lg border border-blue-200">
                                        <p className="text-blue-800 text-sm font-semibold flex items-center">
                                            ...
                                            Modo Férias Ativado! Ritmo super intensivo (11h+ de estudo). Aproveite ao máximo cada hora livre para finalizar a jornada!
                                        </p>
                                    </div>
                                )}
                                ...
                                {!activeDay.isIntensive && !activeDay.isSpecial && !activeDay.isBirthday && !activeDay.isCompleted && !activeDay.isVacation && (
                                    <div className="mt-6 bg-slate-100 p-4 rounded-lg border border-slate-200">
                                        <p className="text-slate-700 text-sm font-medium flex items-center">
                                            <span className="text-xl mr-2">🕒</span>
                                            Ritmo Normal: Foco de 2h 15m com consistência e qualidade.
                                        </p>
                                    </div>
                                )}
                                ...
                                {activeDay.isBirthday && (
                                    <div className="mt-6 bg-pink-100 p-4 rounded-lg border border-pink-300">
                                        <p className="text-pink-800 text-sm font-semibold flex items-center">
                                            <span className="text-xl mr-2">🎂</span>
                                            Feliz Aniversário! Aproveite muito o seu dia de descanso merecido.
                                        </p>
                                    </div>
                                )}
                                
                                {activeDay.isSpecial && (
                                    <div className="mt-6 bg-purple-100 p-4 rounded-lg border border-purple-300">
                                        <p className="text-purple-800 text-sm font-semibold flex items-center">
                                            <span className="text-xl mr-2">⭐</span>
                                            Evento Especial: Um dia diferente na sua jornada. Concentre-se no seu concurso. Boa sorte!
                                        </p>
                                    </div>
                                )}
